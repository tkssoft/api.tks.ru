...
> Здесь будет ссылка на документацию по работе с сервисом сертификатов.
...
> Здесь будет ссылка на документацию по работе с api.tks.ru

> Здесь будет ссылка на документацию по дереву ТН ВЭД

> Здесь будет ссылка на документацию для сервиса подбора кода по наименованию.

## Расчет контракта.

### Описание системы расчета контрактов
...
* Сделать запрос к API ТН ВЭД по коду товара. Документация https://api.tks.ru/static/docs/tnved.json/readme.html . Примерный запрос http://www.tks.ru/tnved.json/json/<сертификат клиента>/<код ТН ВЭД>.json
...
* Отправить запрос на расчет. http://calc.tks.ru/calc/<сертификат_клиента>/
* Получить результаты расчетов.


Входная информация является опциональной (кроме кода ТН ВЭД и таможенной стоимости). Система сначала запрашивает информацию по код ТН ВЭД, а потом дополняет ее теми данными, которые передал пользователь. Такой поход позволяет, например, указать пользователю ставку НДС 10% (NDS: 10)
