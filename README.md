...
Для хранения настроек общего расчета используется поле `totals`. Значения разделены по типам. Использование в расчетах конкретного типа определяется либо фиксировано в программе, либо запрашивается у пользователя. В функцию `process_config` передаются настройки только одного типа. Структура информации по типу расчета содержит следующие поля:

* `name` - имя типа расчета, которое появляется при формировании итогов.
* `type` - имя типа расчета, которое используется при запросе необходимого расчета у пользователя.
* `values` - строки с расчетными данными.

Структура информации в поле `values`:
...
Структура результатов расчетов (результат работы функции `process_config`):
